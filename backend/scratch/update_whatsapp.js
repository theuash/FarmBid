...
// 1. Replace the entire `whatsappMessages` object
const oldMessagesStartStr = `const whatsappMessages = {`;
const oldMessagesEndStr = `// Format time remaining`;

const msgStartIndex = content.indexOf(oldMessagesStartStr);
const msgEndIndex = content.indexOf(oldMessagesEndStr);

if (msgStartIndex !== -1 && msgEndIndex !== -1) {
  const newMessages = `const whatsappMessages = {
  english: [
    { type: 'bot', text: 'Welcome to FarmBid! Please select your language / ದಯವಿಟ್ಟು ನಿಮ್ಮ ಭಾಷೆಯನ್ನು ಆಯ್ಕೆ ಮಾಡಿ / कृपया अपनी भाषा चुनें' },
    { type: 'bot', text: 'Choose Language:', isInteractive: true, options: [' English', ' ಕನ್ನಡ', ' हिंदी'] },
...
`;
  ...
  content = content.substring(0, msgStartIndex) + newMessages + content.substring(msgEndIndex);
} else {
  console.error("Could not find whatsappMessages object");
  process.exit(1);
}

// 2. Update the WhatsAppDemo render logic to support msg.isInteractive
const oldRender = \`}
              >
                {msg.isImage ? (
                  <img
...
const normalizedOldRender = normalize(oldRender);

if (normalizedContent.includes(normalizedOldRender)) {
  content = normalizedContent.replace(normalizedOldRender, normalize(newRender));
} else {
  console.log("Could not find WhatsApp render loop");
  // Let's try splitting to find it
  const idx = normalizedContent.indexOf('{msg.isImage ? (');
  if (idx !== -1) {
    console.log("Found msg.isImage at " + idx);
  } else {
    console.log("Cannot find msg.isImage at all");
  }
  process.exit(1);
}
